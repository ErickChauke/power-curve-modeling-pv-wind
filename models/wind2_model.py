"""Standalone Wind2 power curve model: GP and Gradient Boosting.

No physics formula: the grid is fully dense, so there is no unsampled-region extrapolation
problem to motivate one (see notebook.ipynb's Wind2 Model section). Refits both models from the
source grid on import (the dataset is tiny, refitting is instant). Wind direction is circular, so
it is encoded as (sin, cos) internally rather than fed in as raw degrees. The documented
near-zero negative artifact is clipped to 0 before fitting. Exposes
